...
Python 3.11 is the supported version for this repository. In particular, the
training dependency `bitsandbytes==0.45.4` does not provide Python 3.13 wheels,
so installation in a 3.13 environment will fail.

For evaluation-only workflows on macOS, use the lighter dependency set:
```bash
pip install -r requirements-eval.txt
```
This skips Linux-only packages used for training or local vLLM-based reranking.
