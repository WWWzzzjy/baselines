...
repo_to_top_folder = {
    ...
    'pvlib/pvlib-python': 'pvlib-python', 
...
def get_project_structure_from_scratch(
    ...
    if repo_name not in repo_to_top_folder:
        repo_to_top_folder[repo_name] = repo_name.split('/')[-1]
    ...
    # clean up
    subprocess.run(
        ["rm", "-rf", f"{repo_playground}/{repo_to_top_folder[repo_name]}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True
    )
    d = {
        "repo": repo_name,
