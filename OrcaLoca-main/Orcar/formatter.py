...
def serialize_chat_message(message: ChatMessage) -> dict:
    ...
    return {
        "role": role,
        "content": content,
        "additional_kwargs": _jsonable(message.additional_kwargs),
    }
...
def get_response_token_count(response: ChatResponse) -> Optional[TokenCount]:
    ...
    in_token_cnt = _get_usage_value(usage, "prompt_tokens")
    out_token_cnt = _get_usage_value(usage, "completion_tokens")
    if in_token_cnt is None:
        in_token_cnt = _get_usage_value(usage, "input_tokens")
    if out_token_cnt is None:
        out_token_cnt = _get_usage_value(usage, "output_tokens")

    if in_token_cnt is None or out_token_cnt is None:
        return None
    return TokenCount(in_token_cnt=in_token_cnt, out_token_cnt=out_token_cnt)
...
class TokenCounter:
        ...
        if isinstance(llm, OpenAI):
            ...
            if tokenizer is not None:
                ...
                try:
                    ...
                    logger.warning(
                        "Unknown OpenAI-compatible model '%s' for tiktoken; using cl100k_base.",
                        model,
                    )
    ...
    def count_chat(
        ...
        api_token_count = get_response_token_count(response)
        if api_token_count is not None:
            return response, api_token_count
