...
class SearchWorker(BaseAgentWorker):
    ...
    def __init__(
        ...
        self.last_message_records: List[dict] = []
    ...
    def _class_methods_ranking(
        ...
        if is_class:
            ...
            task.extra_state["message_records"].extend(
                code_scorer.get_call_records("Methods Score")
            )
    ...
    def _file_functions_ranking(
        ...
        if is_file:
            ...
            scores = code_scorer.score_batch(chat_messages)
            task.extra_state["message_records"].extend(
                code_scorer.get_call_records("Functions Score")
            )
            task.extra_state["token_cnts"].append(
    ...
    def _disambiguation_ranking(
        ...
        if is_disambiguation:
            ...
            scores = code_scorer.score_batch(chat_messages)
            task.extra_state["message_records"].extend(
                code_scorer.get_call_records("Disambiguation Score")
            )
            task.extra_state["token_cnts"].append(
    ...
    def _run_step(
        ...
        elapsed_s = time.perf_counter() - call_start
        api_token_count = get_response_token_count(chat_response)
        if api_token_count is not None:
            token_cnt = api_token_count
        ...
        if task.extra_state["is_done"]:
            ...
            stage = "Searcher step"
            task.extra_state["token_cnts"].append((stage, token_cnt))
        task.extra_state["message_records"].append(
            {
                "stage": stage,
                "messages": serialize_chat_messages(input_chat),
        ...
        logger.info(f"{stage} LLM time: {elapsed_s:.2f}s")
