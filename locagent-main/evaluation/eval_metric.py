...
def acc_at_k(pred_target: Tensor, ideal_target: Tensor, k: Optional[int] = None) -> Tensor:
    ...
    relevant = (pred_target == 1).sum(dim=-1).float()  # 计算预测中相关文档的个数
    total_relevant = (ideal_target == 1).sum(dim=-1).float()  # 计算所有相关文档的个数

    # comparison = relevant == total_relevant
    # return comparison.sum()/relevant.shape[0]
    return div_no_nan(relevant, total_relevant).mean(0)
    
def f1_at_k(pred_target: Tensor, ideal_target: Tensor, k: Optional[int] = None) -> Tensor:
    pred_target = pred_target[:, :k]
    
    relevant = (pred_target == 1).sum(dim=-1).float()         # 前k个命中GT的数量
    total_relevant = (ideal_target == 1).sum(dim=-1).float()  # GT总数
    
    precision = relevant / k                                   # P@k = 命中数 / k
    recall = div_no_nan(relevant, total_relevant)              # R@k = 命中数 / GT总数
    
    f1 = div_no_nan(2 * precision * recall, precision + recall)  # F1 调和平均
    return f1.mean(0)
...
def calc_token_usage(traj_file, model_name_or_path="Qwen/Qwen2.5-7B-Instruct"):
    tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
    
    traj_datas = load_jsonl(traj_file)
    results = {}
    
    for data in traj_datas:
        instance_id = data['instance_id']
        total_tokens = 0
        total_time = 0.0
        
        for traj in data['loc_trajs']['trajs']:
            # token 统计
