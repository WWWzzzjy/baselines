...
DEFAULT_TEMPERATURE = float(os.environ.get("LOCAGENT_TEMPERATURE", "0.6"))
DEFAULT_TOP_P = float(os.environ.get("LOCAGENT_TOP_P", "0.95"))
...
def load_instance_ids_file(path: str) -> list[str]:
    if not path:
        return []
    ...
    if path.endswith('.json'):
        loaded_ids = json.loads(raw_content)
        if not isinstance(loaded_ids, list):
            raise ValueError('instance_ids_file JSON must contain a list of instance IDs.')
        return loaded_ids
...
def auto_search_process(result_queue,
    ...
    while not finish:
        ...
        if isinstance(response.choices[0].message.content, str):
            stripped_content = response.choices[0].message.content.strip()
            if stripped_content:
                last_nonempty_assistant_content = stripped_content
        ...
        for action in actions:
            ...
            if action.action_type == ActionType.FINISH:
                ...
                if not final_output:
                    final_output = last_nonempty_assistant_content
                logging.info('='*15)
                logging.info("\nFinal Response:=\n" + final_output)
                finish = True # break
        ...
        for _ in range(args.num_samples):
            ...
            while max_attempt_num:
                ...
                try:
                    ...
                    process.start()
                    try:
                        # Read the child result before joining; otherwise a large payload can
                        # fill the multiprocessing pipe and deadlock the parent on join().
                    ...
                    logger.warning(
                        f'{e}. Remaining attempts: {max_attempt_num}.'
                    )
                    if max_attempt_num <= 0:
                        logger.warning(
                            f'{instance_id} exhausted retries after BadRequestError.'
                        )
                        break
                    continue
                ...
                if not loc_result:
                    max_attempt_num = max_attempt_num - 1
                    logger.warning(
                        f"{instance_id} produced an empty localization result. Remaining attempts: {max_attempt_num}."
                    )
...
def localize(args):
    ...
    if args.instance_ids_file:
        selected_ids = set(load_instance_ids_file(args.instance_ids_file))
        if selected_ids:
            bench_tests = bench_tests.filter(lambda example: example['instance_id'] in selected_ids)
            logging.info(f'Filtering evaluation to {len(bench_tests)} instances from instance_ids_file.')
...
def main():
    ...
    parser.add_argument("--instance_ids_file", type=str, default='',
                        help='Optional file containing selected instance IDs (.json list or one ID per line).')
    ...
    parser.add_argument(
        ...
        help='LiteLLM model string. Plain model names will be normalized to openai/<model>.'
    )
