...
def f1_at_k(pred_target: Tensor, ideal_target: Tensor, k: Optional[int] = None) -> Tensor:
    pred_target = pred_target[:, :k]
    ideal_target = ideal_target[:, :k]
    relevant = (pred_target == 1).sum(dim=-1).float()
    total_relevant = (ideal_target == 1).sum(dim=-1).float()
    precision = div_no_nan(relevant, torch.full_like(relevant, float(k)), na_value=0.)
    recall = div_no_nan(relevant, total_relevant, na_value=0.)
    return div_no_nan(2 * precision * recall, precision + recall, na_value=0.).mean(0)
