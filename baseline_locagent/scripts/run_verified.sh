...
ENV_SCRIPT="${PROJECT_ROOT}/scripts/env/dashscope_verified.sh"
...
if [[ "${PREBUILD_INDICES}" == "1" ]]; then
  ...
  "${PYTHON_BIN}" dependency_graph/batch_build_graph.py "${PREBUILD_GRAPH_ARGS[@]}"
  "${PYTHON_BIN}" build_bm25_index.py "${PREBUILD_BM25_ARGS[@]}"
fi
