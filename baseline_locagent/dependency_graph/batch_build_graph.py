...
def run(rank, repo_queue, repo_path, out_path,
        download_repo=False, instance_data=None, counter=None, lock=None,
        total=0):
    while True:
        try:
        ...
        if osp.exists(output_file):
            ...
            report_progress(
                'graph', counter, lock, total, repo_name, 'skipped',
        ...
        if download_repo:
            ...
            try:
                ...
                report_progress(
                    'graph', counter, lock, total, repo_name, 'checkout_error',
                    time.time() - repo_start_time,
                )
                continue
        else:
        ...
        print(f'[{rank}] Start graph process {repo_name}', flush=True)
        try:
            ...
            report_progress(
                'graph', counter, lock, total, repo_name, 'ok',
                time.time() - repo_start_time,
            )
        except Exception as e:
            print(f'[{rank}] Error processing {repo_name}: {e}', flush=True)
            report_progress(
                'graph', counter, lock, total, repo_name, 'error',
                time.time() - repo_start_time,
            )
...
if __name__ == '__main__':
    ...
    print(f'[graph] Starting graph prebuild for {total_repos} instances with {args.num_processes} processes.', flush=True)
    ...
    end_time = time.time()
    print(f'[graph] Total execution time = {end_time - start_time:.3f}s', flush=True)
